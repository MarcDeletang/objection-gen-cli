...
const createCustomType = async (): Promise<PromptResponse<string>> => abortablePrompts({
    type: 'text',
    name: 'value',
...
const createProperties = async (properties: Property[] = []): Promise<Property[]> => {
...
export const createModels = async (models: Model[] = []): Promise<Model[]> => {
    ...
    while (await yesOrNoPrompts('Do you want to create another model ? Y/n', 'y')) {
        return createModels([...models, { name, table, properties, relations: [] }]);
    }
