...
const generateProperties = (properties) => properties.map(p => `    public ${p.name}${p.optional ? '?' : ''}: ${p.type};`)
    .join('\n');
// TODO later maybe
...
const generateRelations = (currentModel, models) => currentModel.relations.map(r => `            ${r.name}: {
