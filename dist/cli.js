...
(async () => {
    ...
    console.log("1: Setup");
    console.log("2: Models creation (name, properties if you are using typescript)");
    console.log("3: Models relations creation");
    console.log("4: You are done :)");
    const { path, extension } = await setupBuilder_1.createSetup();
    const models = await relationBuilder_1.createRelations(await modelBuilder_1.createModels(extension));
